...
function mpdf_filter($wp_content = '', $do_pdf = false, $convert = false) {
	...
	$wp_content = str_replace('"/wp-content/', '"http://' . $_SERVER['SERVER_NAME'] . '/wp-content/', $wp_content);
...
function mpdf_pdfbutton($opennewtab=false, $buttontext = '<img border="0" alt="This page as PDF" src="http://www.edcwc.com/refresh/wp-content/plugins/wp-mpdf/pdf.png"><span>Download/print this page as a PDF</span>', $logintext = 'Login!', $print_button = true, $nofollow = false, $options = array()) {
	$nofollowHtml = '';
	if($nofollow) {
...
function mpdf_admin_printeditbox() {
	...
	echo '<tr><td>'.__('Ignore this checkbox', 'wp-mpdf').'</td><td><input ';
	if($datas->login == true) {
		echo 'checked="checked" ';
	}
